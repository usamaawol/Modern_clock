...
const Index = () => {
  ...
  const { isListening, transcript, startListening, stopListening, isSupported } = useVoiceCommands();
  ...
  const handleVoiceToggle = () => {
    if (isListening) {
      ...
      if (result.type === 'alarm' && result.data?.time) {
        addAlarm({
        ...
        toast.success(`Alarm set for ${result.data.time}`);
      } else if (result.type === 'task' && result.data?.title) {
        setVoiceTaskTitle(result.data.title);
        setShowAddTask(true);
      } else if (transcript) {
        toast.info('Command not recognized. Try "Set alarm for 7 AM" or "Add task: Exercise"');
      }
    } else {
      startListening();
    }
